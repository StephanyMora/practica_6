...
void variableSeleccionada_RB(){
   ...
   //solo se utiliza cuando hay un retunrn en medio de la funcion
...
int i = 0;
 ...
 for (i=0; i< 8; i++) {
  ...
  while(input(datosDHT)==0){
    delay_us(30);
  }
  if (input(datosDHT)==1)//Was: if(PINC & _BV(dht_PIN))
  {
  result |=(1<<(7-i));
  }
}
