      ...
      if(estadoConexion()){
         ...
         if(checksum == (enteroRH + decimalRH + enteroTemperatura + decimalTemperatura)){
          ...
          //guardo los números de manera adecuada corregir esto
            enteroRH = enteroRH / 10;
            decimalRH = decimalRH % 10;
            enteroTemperatura = enteroTemperatura / 10;
            decimalTemperatura = decimalTemperatura % 10;
            if(mostrarTemperatura == true){
              while(int i < 9){
                int i = 1;
                mostrarTemperatura[i] = enteroTemperatura; 
              }
              output_d(numerosDisplay[mostrarTemperatura[]]);

            }
            else if(mostrarHumedad == true){
               output_d(numerosDisplay[mostrarHumedad[]]);
            }          
         }
      }
   }	
